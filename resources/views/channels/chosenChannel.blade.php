            ...
            <div class="col-md-8" style = "margin-left: 150px;">
                <ul class="news-list half-image">
                    <li>
                        ...
                        @auth()
                        <form action="{{url('/addChannelToFavourite')}}">
                            <input type="hidden" name="channel" value="{{ $channel->name }}">
                            <input type="submit" value="Добавить в любимые">
                        </form>
                        @if (session('message'))
                            <br>
                            <br>
                ...
                <ul class="news-list half-image">
                    @foreach($result as $key =>  $teleprogramms)
                        <h4>{{$key}}</h4>
                    <table>
                        <tr>
                            <td width="150px">
                        ...
                        @foreach($teleprogramms as $teleprogramm)
                            <tr>
                                ...
                                <td>
                                    <h4>{{$teleprogramm->category}}:   {{$teleprogramm->name}}    {{$teleprogramm->age_raing}}</h4>
                                </td>
                                <td width="150px">
